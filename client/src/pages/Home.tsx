...
type SortKey = "noDo" | "customer" | "pool" | "jalurAwalDate" | "slaDays" | "status" | "category";
...
const DATE_LIKE = /^\d{1,2} (January|February|March|April|May|June|July|August|September|October|November|December) \d{4}$/;
const isDateLike = (value: string) => DATE_LIKE.test(value);
...
export default function Home() {
  ...
  const [sortKey, setSortKey] = useState<SortKey>("slaDays");
  ...
  const filteredRecords = useMemo(() => {
    ...
    return data.records.filter((record) => {
      ...
      const haystack = `${record.noDo} ${record.customer} ${record.reason} ${record.pool} ${record.status} ${record.category}`.toLowerCase();
  ...
  useEffect(() => { setPage(1); }, [status, category, year, pool, month, search, pageSize]);
  ...
  const toggleSort = (key: SortKey) => {
    ...
    else { setSortKey(key); setSortDir(key === "slaDays" || key === "jalurAwalDate" ? "desc" : "asc"); }
  };

  const sortIcon = (key: SortKey) => {
    if (key !== sortKey) return <ArrowUpDown className="h-3 w-3 text-slate-300" />;
    return sortDir === "asc" ? <ArrowUp className="h-3 w-3 text-[#2c63d6]" /> : <ArrowDown className="h-3 w-3 text-[#2c63d6]" />;
  };
    ...
    <div className="min-h-screen bg-[#f5f7fb] text-slate-900">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[248px] flex-col bg-[#101b31] text-white transition-transform duration-200 lg:translate-x-0 ${mobileNav ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 px-3 py-7">
          ...
          <button className="mt-2 flex w-full cursor-not-allowed items-center gap-3 rounded-xl px-3 py-3 text-left text-sm text-slate-500" disabled aria-disabled="true" title="Coming soon">
            <BarChart3 className="h-[18px] w-[18px]" />
            Analytics
            <span className="ml-auto rounded-md bg-white/10 px-2 py-0.5 text-[10px] text-slate-400">Soon</span>
          </button>
          <button className="mt-2 flex w-full cursor-not-allowed items-center gap-3 rounded-xl px-3 py-3 text-left text-sm text-slate-500" disabled aria-disabled="true" title="Coming soon">
      ...
      <main className="min-h-screen lg:pl-[248px]">
        <header className="sticky top-0 z-20 flex h-[82px] items-center justify-between border-b border-slate-200/80 bg-[#f5f7fb]/90 px-5 backdrop-blur md:px-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#dce9ff] text-xs font-bold text-[#2756a4]" title="Signed in as Administrator">A</div>
          </div>
        </header>
        ...
        <div className="mx-auto max-w-[1440px] px-5 py-7 md:px-8 md:py-9">
          ...
          <section id="detail-table" className="mt-5 rounded-2xl border border-slate-200/80 bg-white shadow-[0_8px_30px_rgba(16,27,49,0.04)]"><div className="border-b border-slate-100 p-5 md:p-6"><div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-center"><div><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">Case register</p><h2 className="mt-1 text-lg font-bold tracking-tight text-[#101b31]">LKPB detail records</h2><p className="mt-1 text-xs text-slate-400">Showing {filteredRecords.length === 0 ? 0 : pageStart + 1}–{Math.min(pageStart + pageSize, filteredRecords.length)} of {filteredRecords.length} records</p></div><div className="flex flex-col gap-2 sm:flex-row"><div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search DO, customer, pool..." className="h-10 w-full rounded-xl border border-slate-200 bg-[#f8fafc] pl-9 pr-9 text-xs outline-none transition focus:border-[#2c63d6] focus:ring-2 focus:ring-[#2c63d6]/10 sm:w-[210px]" />{search && <button type="button" onClick={() => setSearch("")} className="absolute right-2 top-1/2 inline-flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-md text-slate-400 transition hover:bg-slate-200 hover:text-slate-600" aria-label="Clear search"><X className="h-3.5 w-3.5" /></button>}</div><select value={year} onChange={(event) => setYear(event.target.value)} className="h-10 rounded-xl border border-slate-200 bg-[#f8fafc] px-3 text-xs font-semibold text-slate-600 outline-none focus:border-[#2c63d6]"><option value="ALL">All years</option>{data.years.map((item) => <option key={item} value={item}>{item}</option>)}</select><select value={pool} onChange={(event) => setPool(event.target.value)} className="h-10 max-w-[180px] rounded-xl border border-slate-200 bg-[#f8fafc] px-3 text-xs font-semibold text-slate-600 outline-none focus:border-[#2c63d6]"><option value="ALL">All pools</option>{data.pools.map((item) => <option key={item} value={item}>{item}</option>)}</select><select value={month} onChange={(event) => setMonth(event.target.value)} className="h-10 rounded-xl border border-slate-200 bg-[#f8fafc] px-3 text-xs font-semibold text-slate-600 outline-none focus:border-[#2c63d6]"><option value="ALL">All months</option>{data.months.map((m) => <option key={m} value={m}>{monthLabel(m)}</option>)}</select><select value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)} className="h-10 rounded-xl border border-slate-200 bg-[#f8fafc] px-3 text-xs font-semibold text-slate-600 outline-none focus:border-[#2c63d6]"><option value="ALL">All status</option><option value="OPEN">Open</option><option value="FINISH">Finish</option></select><select value={category} onChange={(event) => setCategory(event.target.value)} className="h-10 rounded-xl border border-slate-200 bg-[#f8fafc] px-3 text-xs font-semibold text-slate-600 outline-none focus:border-[#2c63d6]"><option value="ALL">All category</option>{data.categories.map((item) => <option key={item.name} value={item.name}>{item.name}</option>)}</select><button onClick={resetFilters} className="h-10 rounded-xl border border-slate-200 px-3 text-xs font-semibold text-slate-500 hover:bg-slate-50">Reset</button></div></div></div><div className="overflow-x-auto"><table className="w-full min-w-[860px] border-collapse text-left"><thead><tr className="border-b border-slate-100 bg-[#fbfcfe] text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">{[ {key: "noDo", label: "No. DO"}, {key: "customer", label: "Customer"}, {key: "pool", label: "Pool"}, {key: "jalurAwalDate", label: "Jalur awal"}, {key: null, label: "Reinstall"}, {key: "slaDays", label: "SLA"}, {key: "status", label: "Status"}, {key: "category", label: "Category"}, {key: null, label: "Reason"}, ].map((col) => <th key={col.label} className="px-5 py-3.5 md:px-6">{col.key ? <button type="button" onClick={() => toggleSort(col.key as SortKey)} className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 transition-colors hover:text-slate-700">{col.label}{sortIcon(col.key as SortKey)}</button> : col.label}</th>)}</tr></thead><tbody>{paginatedRecords.map((record) => <tr key={`${record.no}-${record.noDo}`} className="border-b border-slate-50 last:border-0 hover:bg-[#fbfcfe]"><td className="whitespace-nowrap px-5 py-4 font-mono text-xs font-semibold text-[#2c63d6] md:px-6">{record.noDo}</td><td className="whitespace-nowrap px-5 py-4 text-xs font-semibold text-slate-700">{record.customer}</td><td className="whitespace-nowrap px-5 py-4 text-xs font-semibold text-slate-500">{record.pool}</td><td className="whitespace-nowrap px-5 py-4 text-xs text-slate-500">{isDateLike(record.jalurAwal) ? record.jalurAwal : <span className="text-slate-300" title="Tanggal jalur awal tidak tersedia di spreadsheet">—</span>}</td><td className="whitespace-nowrap px-5 py-4 text-xs text-slate-500">{record.reinstall ? (isDateLike(record.reinstall) ? record.reinstall : <span className="inline-block rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-500" title="Status reinstall (bukan tanggal)">{record.reinstall}</span>) : <span className="text-slate-300">—</span>}</td><td className="whitespace-nowrap px-5 py-4"><span className={`inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-[11px] font-bold ${record.slaDays >= 30 ? "bg-[#fff0ed] text-[#c64f35]" : record.slaDays >= 15 ? "bg-[#fff9e8] text-[#a17800]" : "bg-[#eaf8f1] text-[#17824d]"}`}><Clock3 className="h-3 w-3" />{record.sla}</span></td><td className="whitespace-nowrap px-5 py-4"><span className={`inline-flex items-center gap-1.5 text-xs font-bold ${record.status === "OPEN" ? "text-[#ad7d00]" : "text-[#18834d]"}`}><span className={`h-1.5 w-1.5 rounded-full ${record.status === "OPEN" ? "bg-[#f5c84b]" : "bg-emerald-500"}`} />{record.status}</span></td><td className="whitespace-nowrap px-5 py-4"><span className="rounded-md bg-slate-100 px-2 py-1 text-[10px] font-bold tracking-wide text-slate-600">{record.category}</span></td><td className="max-w-[260px] truncate px-5 py-4 text-xs text-slate-500" title={record.reason}>{record.reason}</td></tr>)}</tbody></table>{paginatedRecords.length === 0 && <div className="px-6 py-12 text-center"><Search className="mx-auto h-6 w-6 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-600">Tidak ada record yang cocok</p><button onClick={resetFilters} className="mt-2 text-xs font-bold text-[#2c63d6] hover:underline">Reset filter</button></div>}</div><div className="flex flex-col items-center justify-between gap-3 border-t border-slate-100 px-5 py-4 text-[11px] text-slate-500 sm:flex-row md:px-6"><div className="flex items-center gap-2"><span>Rows per page</span><select value={pageSize} onChange={(event) => setPageSize(Number(event.target.value))} className="h-8 rounded-lg border border-slate-200 bg-[#f8fafc] px-2 text-xs font-semibold text-slate-600 outline-none focus:border-[#2c63d6]">{PAGE_SIZE_OPTIONS.map((n) => <option key={n} value={n}>{n}</option>)}</select></div><div className="flex items-center gap-2"><span>Page {safePage} of {pageCount}</span><button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={safePage <= 1} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Previous page"><ChevronLeft className="h-4 w-4" /></button><button type="button" onClick={() => setPage((p) => Math.min(pageCount, p + 1))} disabled={safePage >= pageCount} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Next page"><ChevronRight className="h-4 w-4" /></button></div></div><div className="flex flex-col items-start justify-between gap-3 border-t border-slate-100 px-5 py-4 text-[11px] text-slate-400 sm:flex-row sm:items-center md:px-6"><span>Source: Google Sheets / Detail LKPB</span><a href={data.sourceUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 font-semibold text-[#2c63d6] hover:underline">View source sheet <ExternalLink className="h-3 w-3" /></a></div></section>
